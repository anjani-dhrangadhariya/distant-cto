'''
Model with SCIBERT as embedding layer followed by BiLSTM layer for inputs and POS (without attention weights) and a final CRF decoder
'''
...
class SCIBERTPOSCRF(nn.Module):

    def __init__(self, freeze_bert, tokenizer, device, bidirectional):
        super(SCIBERTPOSCRF, self).__init__()
        ...
        if bidirectional == True:
            self.lstmpos_layer = nn.LSTM(input_size=44, hidden_size = 20, num_layers = 1, bidirectional=bidirectional, batch_first=True)
            self.lstm_layer = nn.LSTM(input_size=808, hidden_size = 512, num_layers = 1, bidirectional=bidirectional, batch_first=True)
        elif bidirectional == False:
            self.lstmpos_layer = nn.LSTM(input_size=44, hidden_size = 20, num_layers = 1, bidirectional=bidirectional, batch_first=True)
            self.lstm_layer = nn.LSTM(input_size=788, hidden_size = 512, num_layers = 1, bidirectional=bidirectional, batch_first=True)
