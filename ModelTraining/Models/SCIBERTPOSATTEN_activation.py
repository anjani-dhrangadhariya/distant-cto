...
class SCIBERTPOSAttenActLin(nn.Module):
    ...
    def __init__(self, freeze_bert, tokenizer, device, bidirectional):
        super(SCIBERTPOSAttenActLin, self).__init__()
    ...
    def forward(self, input_ids=None, attention_mask=None, labels=None, input_pos=None):
        ...
        # log reg
        # print(self.hidden2tag( lstm_output )[0][0])
        probablity = self.sigmoid( self.tanh( self.hidden2tag( lstm_output ) ) ) 
        # print(probablity[0][0])
        max_probs = torch.max(probablity, dim=2) # get the highest of two probablities
        # print(max_probs[0][0])
        logits = max_probs.indices.flatten()
        # print(max_probs.indices[0])
        # print('-------------------------------------------------')
